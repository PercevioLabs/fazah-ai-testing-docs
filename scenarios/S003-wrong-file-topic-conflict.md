...
```text
Explain functional and non-functional requirements using the selected slides.
```
...
```text
So which file should I use for this?
```
...
```text
Use this Requirements file instead.
```
...
```text
Now explain functional versus non-functional requirements correctly from it.
```
...
```text
Give me a Mentcare example of each.
```
...
```text
Put functional versus non-functional into a comparison table.
```
...
```text
Make five questions from that table for students, with no answers shown.
```
