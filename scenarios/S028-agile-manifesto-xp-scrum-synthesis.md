...
```text
How do XP practices support those values?
```
...
```text
Now turn that table into three quick-check questions for students.
```
...
```text
Add a teacher key for those questions.
```
...
```text
Confirm all of this came from the Agile deck.
```
