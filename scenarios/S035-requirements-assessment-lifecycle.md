...
```text
Create six short-answer questions on requirements engineering, with answers.
```
...
```text
Replace question 3 with a scenario question.
```
...
```text
Remove any question about requirements management.
```
...
```text
Add one requirements-validation question, keeping six questions in total.
```
...
```text
Reorder the questions from foundational to advanced.
```
...
```text
Now give me a clean student version with no answers.
```
...
```text
Create a separate teacher answer key without changing the student version.
```
...
```text
Turn one of the questions into multiple choice.
```
...
```text
Which file did you use, and list everything we made.
```
