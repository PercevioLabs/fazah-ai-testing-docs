...
```text
Create a 20-point exam with three sections and a mix of question types. Balance the coverage across
the three selected files and include a teacher answer key.
```
...
```text
Verify that the total adds up to 20 points.
```
...
```text
Rebalance the coverage so all three files are represented evenly.
```
...
```text
Label each question with the source file it came from.
```
...
```text
Now make a student version with no answers.
```
...
```text
Produce the teacher answer key as a separate document.
```
...
```text
Confirm the coverage is balanced across the three files.
```
