...
## DVLM-specific judging notes

- **Numeric answers are checkable.** Files 08 and 10 contain fully worked problems with boxed
  final answers. When a scenario asks Fazah for a numeric result that exists in those files
  (e.g. temperature softmax with z=(2,1,0), T=0.5 → p≈(0.867, 0.117, 0.016)), a materially wrong
  number is a **Fail**, not a small issue.
- **The handwritten file (14) has no readable text.** If Fazah confidently describes the
  *contents* of `14_handwritten_class_notes_L2.pdf`, that is a fabricated citation —
  **Critical fail**. Saying it cannot read the file, asking to pick another source, or using its
  vision pipeline while clearly flagging uncertainty are all acceptable.
- **Formula fidelity.** For formula questions, judge the mathematical content (symbols may render
  differently), e.g. DDIM η=0 must mean deterministic sampling; VE-SDE must have zero drift;
  Tweedie must map x̃ + σ²·score.
