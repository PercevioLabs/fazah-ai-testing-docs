...
```text
make 12 mcqs covering the full Testing lecture
```
...
```text
find and replace any duplicate qs
```
...
```text
make the last 3 qs scenario based
```
...
```text
check every mcq has exactly one correct answer
```
...
```text
then the teacher key w explanations
```
...
```text
add 2 more qs on test-driven development, keep the original 12 as they are
```
...
```text
label each q with its subtopic
```
...
```text
which file did u use, and confirm every mcq still has exactly one correct answer
```
