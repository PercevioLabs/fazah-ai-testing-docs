...
```text
ok can u compare waterfall vs incremental development for me
```
...
```text
ok now gimme one realistic project situation where each approach fits, but stick to how the lecture frames them
```
...
```text
also add one limitation of each
```
...
```text
make 4 questions from this comparison
```
...
```text
now gimme a student version w no answers
```
...
```text
which file did u use for all this
```
