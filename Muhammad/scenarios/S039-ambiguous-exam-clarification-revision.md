...
```text
maybe cover software processes, requirements engineering, and testing
```
...
```text
for undergraduate students i think
```
...
```text
lets make it 30 points across 3 sections
```
...
```text
ok use these 3 files and balance the coverage across them
```
...
```text
one of the qs feels too easy, maybe swap it for something harder
```
...
```text
whats the point total now, does it add up to 30?
```
...
```text
label each q with the file it came from
```
...
```text
list every q with its source and confirm coverage is balanced across the 3 files
```
